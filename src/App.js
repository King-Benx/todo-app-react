...
function App() {
  ...
  const handleFormSubmit = (data) => {
   const updateData = [...state.data,{ id: state.data.length += 1, item: data, completed:false }];
  ...
  const handleCompleted = () => {
    let filteredData = state.data.filter(it => it.completed !== true);
    const updatedData = [];
  ...
  return (
    <div className="container">
      ...
      <ListSection data={state.data}  itemDeleteListener={handleDeletedItem} itemCheckedListener={handleChecked} editItemListener={editItem} />
      <div className="container-footer">
        <button id="clear-list" onClick={handleCompleted}>Clear all Completed</button>
      </div>
    </div>
  );
}
